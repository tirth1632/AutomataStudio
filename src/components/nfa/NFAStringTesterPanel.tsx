...
import { Play, Check, X, Layers, PlayCircle, Trash2, RotateCcw, Sparkles, BookOpen, GitBranch } from 'lucide-react';
...
interface NFAStringTesterPanelProps {
  nfa?: any;
...
export const NFAStringTesterPanel: React.FC<NFAStringTesterPanelProps> = ({
  nfa: _nfa,
  graph: graphProp,
  promptDescription,
}) => {
  const { graph: globalGraph, setInputString, runSimulation } = useAutomata();
  const graph: AutomatonGraph = graphProp || globalGraph;
  const question = promptDescription || graph?.name || 'NFA pattern matching & nondeterminism validation';
  ...
  // Execute simulation for a given input string on NFA graph
  const testString = useCallback(
    (inputStr: string): TestResult => {
      ...
      const finalStates = lastStep ? lastStep.currentStateIds : [];

      const path: string[] = [];
      steps.forEach((step) => {
        if (step.currentStateIds.length > 0) {
          path.push(`{${step.currentStateIds.join(', ')}}`);
        }
      });
  ...
  // Automatically update target question and sample inputs whenever graph/question changes
    ...
    <div className="p-4 sm:p-5 space-y-4 bg-slate-900/95 text-slate-100 rounded-2xl border border-slate-800 text-sm shadow-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 gap-2 flex-wrap">
        <div className="flex items-center gap-2.5 font-bold text-base text-sky-300">
          ...
          NFA String Tester & Path Validator
        </div>
      ...
      <div className="px-3.5 py-2.5 bg-slate-950/80 border border-sky-500/30 rounded-xl flex items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 min-w-0">
          <BookOpen className="w-4 h-4 text-sky-400 shrink-0" />
          <span className="text-slate-400 font-semibold shrink-0">Target Question:</span>
          <span className="font-mono font-bold text-sky-200 truncate">"{question}"</span>
        </div>
        <div className="flex items-center gap-1.5 text-[11px] text-sky-300 bg-sky-950/60 border border-sky-500/30 px-2 py-0.5 rounded-lg shrink-0 font-medium">
      ...
      {mode === 'single' ? (
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3.5 py-2 bg-slate-950 border border-slate-700/80 rounded-xl font-mono text-sm text-white placeholder-slate-500 focus:outline-none focus:border-sky-500"
          />
          <button
            onClick={handleTestSingle}
            className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-xl text-xs flex items-center gap-1.5 transition shadow"
          >
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            placeholder="101&#10;1101&#10;000101&#10;111"
            className="w-full p-3 bg-slate-950 border border-slate-700/80 rounded-xl font-mono text-sm text-white placeholder-slate-500 focus:outline-none focus:border-sky-500 resize-none"
          />
          <button
            onClick={handleTestBatch}
            className="w-full py-2 bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-500 hover:to-indigo-500 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition shadow"
          >
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-slate-400 font-mono truncate flex items-center gap-1.5">
                    ...
                    <span>Final States: <strong className="text-sky-300">{`{${res.finalStates.join(', ')}}`}</strong></span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  ...
                  title="Replay NFA simulation in visualizer"
                  className="p-1.5 bg-slate-800 hover:bg-sky-600 text-slate-300 hover:text-white rounded-lg transition text-xs flex items-center gap-1"
                >
