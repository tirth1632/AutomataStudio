...
interface NFALanguageExplanationPanelProps {
...
export const NFALanguageExplanationPanel: React.FC<NFALanguageExplanationPanelProps> = ({
  graph: graphProp,
  nfa: _nfa,
  promptDescription,
}) => {
  const { apiKeys, aiProvider, setAIProvider, graph: globalGraph } = useAutomata();
  const graph = graphProp || globalGraph;
  const question = promptDescription || graph?.name || 'NFA pattern matching invariants';
    ...
    async (overrideProvider?: AIProvider) => {
      ...
      if (hasAnyApiKey) {
        ...
        try {
          ...
          console.warn('[NFALanguageExplanationPanel] AI explanation fetch error:', err);
        } finally {
  ...
  return (
    <div className="p-5 sm:p-6 space-y-5 bg-slate-900/95 text-slate-100 rounded-2xl border border-slate-800 text-sm sm:text-base shadow-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-3.5 gap-3 flex-wrap">
        <div className="flex items-center gap-2.5 font-bold text-base sm:text-lg text-sky-300">
          <BookOpen className="w-5 h-5 text-sky-400 shrink-0" />
          NFA Language Explanation & Nondeterminism Invariants
        </div>
        ...
        <div className="relative">
          <button
            ...
            className="px-3.5 py-1.5 bg-sky-950/90 hover:bg-sky-900/90 border border-sky-500/40 text-sky-200 rounded-full text-xs font-bold flex items-center gap-2 shadow-sm transition cursor-pointer"
          ...
          >
            {isAiLoading ? (
              ...
              <>
                ...
                <ChevronDown className="w-3 h-3 text-sky-300 ml-0.5" />
              </>
            )}
          </button>
              ...
              {MODEL_OPTIONS.map((m) => (
                <button
                  ...
                  className={`w-full text-left px-2.5 py-2 rounded-xl transition flex items-center justify-between ${
                    aiProvider === m.id
                      ? 'bg-sky-950 border border-sky-500/40 text-sky-200 font-bold'
                      : 'hover:bg-slate-900 text-slate-300 font-medium'
                  }`}
                >
                  <span>{m.label}</span>
                  {aiProvider === m.id && <Check className="w-3.5 h-3.5 text-sky-400" />}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="p-4 bg-slate-950/90 border border-slate-800 rounded-xl space-y-2">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isAiLoading ? 'animate-spin text-sky-400' : ''}`} />
            Re-generate with AI
          </button>
        </div>
        <div className="text-sky-200 text-sm sm:text-base font-bold font-mono">
          "{question}"
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-950/90 border border-slate-800 rounded-xl space-y-1.5">
        <div className="text-xs font-bold text-sky-400 uppercase tracking-wider">Formal Language Definition (L)</div>
      ...
      <div className="space-y-3">
        <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">
          State-by-State Invariants & Nondeterministic Meanings
        </div>
